...
class Bird():
    def __init__(self, pos=[0, 0], vlc=[0, 0], nose_color=(0, 0, 255)):
        self.pos = pos
        self.vlc = vlc
        self.cruis_vlc = 5  # pixels per second
        self.nose_color = nose_color

    def frame(self, birds, delta_t, w, h):
        forse_out = 0.5  # forse that repels
        forse_in = 0.2
        birds = sorted(birds, key=lambda bird: get_dist(
            bird.pos, self.pos))

        bird_close = birds[0]
        d = get_dist(self.pos, bird_close.pos) + 0.00001  # != 0
        if d < 40:
            self.vlc = [
                self.vlc[i] - forse_out*(bird_close.pos[i] -
                                         self.pos[i])/d for i in [0, 1]]

        bird_far = birds[-1]
        d = get_dist(self.pos, bird_far.pos) + 0.00001  # != 0
        if d > 60:
            self.vlc = [
                self.vlc[i] + forse_in*(bird_far.pos[i] -
                                        self.pos[i])/d for i in [0, 1]]

        n = 1  # 1/speed stabilisator strengh
        vlc_module = hypo(*self.vlc)
